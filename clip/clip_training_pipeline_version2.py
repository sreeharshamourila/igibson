...
def main():
    ...
    for epoch in range(epochs):
        ...
        for batch in data_loader:
            images,texts=batch
            #code to change the second half of the batch into unaligned pairs
            texts_new=[""]*len(texts)
            ##label creation
            labels=[]
            for i in range(0,len(texts)):
                if(i<len(texts)//2):
                    ...
                    r = list(range(1,i)) + list(range(i+1, len(texts)))
                    rand_i=random.choice(r)
                    texts_new[i]=texts[rand_i]
                    #negative sample
                    labels.append(0)
            ##preprocessing images and texts
            images2= torch.stack([transform(img) for img in images],dim=0)
            texts2 = clip.tokenize(texts_new)
            optimizer.zero_grad()
            images2=images2.cuda()
            texts2=texts2.cuda()
            ###tensorizing the labels
            if device == "cpu":
                ground_truth = torch.tensor(labels).long().to(device)
            else:
                ground_truth = torch.tensor(labels).long().to(device)

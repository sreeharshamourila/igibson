...
def get_loader(root, json, mode,transform, batch_size, shuffle, num_workers):
    ...
    coco = CocoDataset(root=root,
                       ...
                       transform=transform)
    print(len(coco))
    train_set,test_set,remaning_set=torch.utils.data.random_split(coco,[30000,10000,374113])
...
def main():
    ...
    optimizer = optim.Adam(model.parameters(), lr=1e-6,betas=(0.9,0.98),eps=1e-7,weight_decay=0.5)
    print(len(train_loader)) 
    epochs=20
